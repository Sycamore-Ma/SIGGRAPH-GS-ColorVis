...
RENDER_RATE = 0.8  # 渲染点数比例

# 3840*2160, 1920*1080, 1080*1080, 720*480
RENDER_WIDTH = 1440
RENDER_HEIGHT = 1440
...
# 2.8, 2.0, 2.0
CAM_DIS_EXTEND = 1.2  # 相机距离点云最大距离的扩展比例

# TRIM_ALPHA = True  # 是否裁剪透明度（仅在高斯点云时使用）
TRIM_ALPHA = False  # 是否裁剪透明度（仅在高斯点云时使用）

# model_name = "trex"  # 模型名称
# ply_paths = [
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/ours_{model_name}_1.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/ours_{model_name}_2.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/ours_{model_name}_3.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/ours_{model_name}_4.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/motion_{model_name}_1.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/motion_{model_name}_2.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/motion_{model_name}_3.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/motion_{model_name}_4.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/seg_{model_name}_1.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/seg_{model_name}_2.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/seg_{model_name}_3.ply",
#     f"E:/SIGGRAPH-GS-ColorVis/ply/{model_name}_PLY/seg_{model_name}_4.ply",    
# ]
...
# === 配置路径 ===
def get_output_path(ply_path = None):
    ...
    if ply_path is not None:
        suffix = ply_path.split('/')[-1].split('.')[0]  # 获取文件名后缀
        output_filename = f"{suffix}_blender_render_{timestamp}.png"  # 输出图片文件名
...
def run_once():
    ...
    for sid in unique_ids:
        ...
        rate = RENDER_RATE / cnt * MAX_CACHE_CNT_OF_POINTS
        print("cnt: ", cnt)

        # if cnt < MAX_CACHE_CNT_OF_POINTS:
        #     rate = 1.0
    ...
    color_map = {sid: colors[i] for i, sid in enumerate(actual_ids)}
    ...
    for sid in set(seg_ids):
        # mat_name = f"Mat_{sid}"
        mat_name = f"Mat_{sid}_{ply_path.split('/')[-1].split('.')[0]}_{seg_key}"
        if mat_name not in bpy.data.materials:
            ...
            # if TRIM_ALPHA:
            #     principled.inputs["Alpha"].default_value = OVERALL_ALPHA / len(ply_paths) * (i+1)
    ...
    for i in range(len(points)):
        ...
        if GAUSSIAN_ON:
            ...
            instance.scale = Vector((1.0, 1.0, 1.0))  
            instance.rotation_mode = 'QUATERNION'
            instance.rotation_quaternion = Vector(qua.tolist())
        ...
        # print(f"Instance_{i} added at {pt.tolist()} with scale {instance.scale} and alpha {alp}")
    ...
    if MANUAL_CAM_ATTRIBUTE is False:
        ...
        center = center - Vector((0, 0, 0.3))  # 稍微抬高一点，避免遮挡
        ...
        cam.location = center + Vector((-cam_distance, -cam_distance, cam_distance*0.75))
        # cam.location = center + Vector((-cam_distance, -cam_distance / 2, cam_distance*0.75))
        # cam.location = center + Vector((-cam_distance, cam_distance, cam_distance*0.75))
        # cam.location = center + Vector((cam_distance, cam_distance, cam_distance*0.75))
        ...
        max_range = np.max(points_np.max(axis=0) - points_np.min(axis=0))  # 点云尺寸
        cam = bpy.context.scene.camera
        cam_distance = max_range * 2.8  # 可根据需要扩大比例
        center = sum((Vector(p.tolist()) for p in points), Vector()) / len(points)
    ...
    bpy.context.scene.render.filepath = get_output_path(ply_path)           # 设置输出路径
    ...
    # # 最后加一句清理垃圾
    # # 内存未释放是导致卡死的核心原因
    # # 只用 bpy.ops.object.delete 是不够的，还需要手动清除 .data 和 bpy.data.xxx.remove()
    # import gc   
    # gc.collect()

    print('successfully render and save image to', get_output_path(ply_path))
...
#     print('before render', i, 'ply_path:', ply_path)    
#     run_once()
#     print('after render', i, 'ply_path:', ply_path)


# def init_and_run_once(ply_path_, seg_key_, gaussian_on):
#     ply_path = ply_path_
#     seg_key = seg_key_
#     GAUSSIAN_ON = gaussian_on
#     print('before render', ply_path)
#     run_once()
#     print('after render', ply_path)


import sys

# Blender默认的sys.argv前面有blender自己的参数，真正的脚本参数从"--"后开始
argv = sys.argv
argv = argv[argv.index("--") + 1:]  # 获取我们真正传入的参数

ply_path = argv[0]
seg_key = argv[1]
GAUSSIAN_ON = argv[2].lower() == "true"  # 字符串转布尔值

print(f">>> PLY: {ply_path}")
print(f">>> SEG_KEY: {seg_key}")
print(f">>> GAUSSIAN_ON: {GAUSSIAN_ON}")

# 然后继续运行你之前的渲染逻辑，使用 ply_path, seg_key, GAUSSIAN_ON
